$(document).ready(function () {
    ...
    $(".side-menu-item-title").bind('click', function () {
        ...
        if ($(this).next('ul').height() == 0) {
            ...
            $(".side-menu-small-item-icon:eq(" + i + ")").next('ul').css({
                "height": "auto",
                "display": "block",
    ...
    $(".side-menu-small-item-icon").bind('click', function () {
        ...
        if ($(this).next('ul').height() == 0) {
            ...
            $(this).next('ul').css({"height": "auto", "display": "block", "max-height": maxHeight + 'px'});
            $(".side-menu-item-title:eq(" + i + ")").next('ul').css({
                "height": "auto",
                "display": "block",
                "max-height": maxHeight + 'px'
            });
    ...
    createTab = function (obj) {
        if (!checkMenuName(obj.title)) {
            var button = "";
            if (obj.isShowClose) {
                button = "<a href='javascript:;' class='system-tab-item-del-button'><i class=\"iconfont icon-close1\"></i></a>"
            }
            clearTabStyle();
            $(".system-tab-nav ul").append('<li data-name="' + obj.title + '" class="system-tab-nav-item system-tab-nav-item-selected">' + obj.title + button + '</li>');
    ...
    // 为每一个tab选项卡动态设置鼠标右击事件
    var flashLiId;
    $('.system-tab-nav ul').on('mouseup', 'li', function (ev) {
        var liid = $('.system-tab-nav ul li').index($(this));
        flashLiId = liid;
        // 如果是鼠标右击，就弹出选项菜单
        if (ev.button == 2) {
            var top = $('.system-tab-nav ul li:eq(' + liid + ')').offset().top + 42;
            var left = $('.system-tab-nav ul li:eq(' + liid + ')').offset().left;
            var tabMenu = $(".system-tab-menu");
            $(tabMenu).hide();
            $(tabMenu).show();
            $(".system-tab-menu").css({"top": top + "px", "left": left + "px"});
        }
    });
    $(".system-tab-menu ul").on("click", "li", function (ev) {
        var index = $(".system-tab-menu ul li").index($(this));
        switch (index) {
            case 0:
                $('.system-content-show:eq(' + flashLiId + ') iframe').attr('src', $('.system-content-show:eq(' + flashLiId + ') iframe').attr('src'));
                break;
            case 1:
                $('.system-tab-nav ul li:eq(' + flashLiId + ') a').click();
                break;
            case 2:
                var ls = $('.system-tab-nav ul li').length;
                for (var i = flashLiId + 1; i < ls; i++) {
                    $('.system-tab-nav ul li:eq(' + (flashLiId + 1) + ') a').click();
                }
                break;
            case 3:
                for (var i = 0; i < flashLiId - 1; i++) {
                    $('.system-tab-nav ul li:eq(' + (flashLiId - 1) + ') a').click();
                }
                break;
            case 4:
                ...
                var tag=0;
                for (var i = 0; i < ls; i++) {
                    if(!$('.system-tab-nav ul li:eq(' + i + ') a').length && $('.system-tab-nav ul li:eq(' + i + ') a').length<=0){
                        tag++;
                    }else{
                        $('.system-tab-nav ul li:eq(' + tag + ') a').click();
                        ls--;
                        i--;
                    }
                }
    ...
    $("body").bind("click", function (ev) {
        $(".system-tab-menu").hide();
        $(".system-tab-skin").hide();
        ev.stopPropagation();
    });
    //为每一个tab选项卡动态设置点击事件
    ...
    $(".side-menu-small-item-icon,.side-menu-small-item ul li").bind('mouseover', function () {
        ...
        $('.stable-left-toast').css({
            "display": "block",
            "padding": (height - $('.stable-left-toast').height()) / 2 + 'px',
            "left": $(this).width() + 'px',
            "top": withBodyHeight - 1 + 'px'
        });
    });
    ...
    // 皮肤面板点击逻辑开始
    $(".system-layout-skin").click(function (ev) {
        var top=67+'px',left=$(this).offset().left+'px';
        $(".system-tab-skin").hide();
        $(".system-tab-skin").css({"top":top,"left":left});
        $(".system-tab-skin").show();
        ev.stopPropagation();
    });
    // 皮肤面板点击逻辑结束

    // 皮肤点击事件开始
    $(".system-tab-skin ul li").click(function (ev) {
        $(".system-tab-skin ul li i").remove();
        $(this).append("<i class=\"iconfont icon-check2\"></i>");
        //皮肤更换逻辑自己定义，请封装成一个方法在此处调用
        ev.stopPropagation();
    });
    // 皮肤点击事件结束
    $(".system-layout-settings").click(function (ev) {
        // layer.open({
        //
        // });
        //自己定义
